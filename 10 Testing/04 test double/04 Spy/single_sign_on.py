...
class MockSingleSignOnRegistry:
    def __init__(self, expected_token, token_is_valid=True):
        ...
        self.is_vlaid_was_called = False

    def is_valid(self, token):
        self.is_valid_was_called = True
        if not token == self.expected_token:
            raise Exception(
                "This mock was given an unexpected argument, Expected {0} got"
                " {1}".format(self.expected_token, token))
        return self.token_is_valid


class SpySingleSignOnRegistry:

    def __init__(self, accept_all_token=True):
        self.accept_all_token = accept_all_token
        self.checked_tokens = []

    def is_valid(self, token):
        self.checked_tookens.append(token)
        return self.accept_all_token
